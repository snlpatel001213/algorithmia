...
def findBreakPoints(Yarray, difference):
    ...
    for i in range(1, len(difference) - 1):
        ...
        thredshold = difference[i]*10/(difference[i - 1]+difference[i + 1])/2
        if (difference[i - 1] > difference[i] < difference[i + 1] ) and thredshold<60:
            # interMediatePoints.append(i)
            lastPoint = i
            subGraphPart.append(interMediatePoints)
            interMediatePoints = []
            print "low Diffrernce point Identified", i + 1
        if (difference[i - 1] < difference[i] > difference[i + 1]) and thredshold>140:
            lastPoint = i
            # interMediatePoints.append(i)
            subGraphPart.append(interMediatePoints)
            interMediatePoints = []
